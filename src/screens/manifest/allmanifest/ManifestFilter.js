...
const ManifestFilter = () => {
    ...
    const getOriginBranch = () =>{
      ...
      .get(
        ServerAddress +
        `master/all-branches/?search=${origin_branch_page_search}&p=${origin_branch_page}&records=${10}&branch_name=${[
            ,
          ]}&branch_city=${[]}&vendor=${[]}&branch_type=${""}&organization=${""}&data=&value=${""}`,
          {
    ...
    const getDestinationBranch = () =>{
      ...
      .get(
        ServerAddress +
        `master/all-branches/?search=${destination_br_page_search}&p=${destination_br_page}&records=${10}&branch_name=${[
            ,
          ]}&branch_city=${[]}&vendor=${[]}&branch_type=${""}&organization=${""}&data=&value=${""}`,
          {
            headers: { Authorization: `Bearer ${accessToken}` },
